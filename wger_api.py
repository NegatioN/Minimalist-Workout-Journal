...
class WgerAPI:
    ...
    def get_api(self, endpoint, action="", options=""):
        ...
        return json.loads(res.content)["results"] if res.status_code == 200 else defaultdict(list)

    def post_api(self, endpoint, data, action="", options=""):
        # needs trailing /, base redirects, and gives us a get-call instead.
        call_url = self.make_my_call_url(action, endpoint, options)
        headers = {'Accept': 'application/json', 'Content-Type': 'application/json'}
        res = post(call_url, json=data, auth=self.auth, headers=headers)
        return json.loads(res.content) if res.status_code == 201 else defaultdict(list)

    def get_exercises(self):
        return self.get_api(endpoint="exercise", options="?limit=1000&language=2")
    ...
    def get_workoutlog(self):
        return self.get_api(endpoint="workoutlog/")
    ...
    def get_workoutsessions(self):
        return self.get_api("workoutsession", options="?limit=9999")

    def post_workoutsession(self, session_data):
        self.post_api(endpoint="workoutsession/", data=session_data)

    def post_workout(self, comment=""):
        return self.post_api(endpoint="workout/", data=wdata.create_workout(comment))

    def get_workouts(self):
        return self.get_api(endpoint="workout", options="?limit=9999")